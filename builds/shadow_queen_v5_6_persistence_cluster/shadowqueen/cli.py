...
from .persistence import Node,seed_demo
def demo(a):
    with tempfile.TemporaryDirectory() as td: print(json.dumps(seed_demo(td),indent=2))
def status(a): print(json.dumps(Node(a.db,a.node).stats(),indent=2))
def evidence(a): print(json.dumps(Node(a.db,a.node).bundle(a.output),indent=2))
def main(argv=None):
    p=argparse.ArgumentParser(); p.add_argument("--db",default="cluster.db"); p.add_argument("--node",default="office:north")
